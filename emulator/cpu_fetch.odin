...
fetch_data :: proc(cpu: ^CPU, cart: ^Cart) {
    ...
    addr := cpu_read_reg(cpu, cpu.cur_inst.reg_2)
            ...
            cpu.fetched_data = u16(bus_read(cart, addr))
            emu_cycles(1)

        } return

        case .AM_R_HLI:
            cpu.fetched_data = u16(bus_read(cart, cpu_read_reg(cpu, cpu.cur_inst.reg_2)))
            emu_cycles(1)
        ...
        case .AM_A8_R:
            cpu.mem_dest = u16(bus_read(cart, cpu.regs.pc)) | 0xFF00
            cpu.dest_is_mem = true
            emu_cycles(1)
            cpu.regs.pc += 1
            return
    ...
    lo := u16(bus_read(cart, cpu.regs.pc))
        ...
        case .AM_MR:
            cpu.mem_dest = cpu_read_reg(cpu, cpu.cur_inst.reg_1)
            cpu.dest_is_mem = true
            cpu.fetched_data = u16(bus_read(cart, cpu_read_reg(cpu, cpu.cur_inst.reg_1)))
            emu_cycles(1)
            return
